...
- **Sam Nasty** files. These are the game and/or editor themselves, generated by stosbuild in a subfolder of the `build` directory.  
  so once generated by *stosbuild* from the *Sam Nasty Template* or source files, you can redistribute the whole `build` subfolder only,  
  without any other files or folders.
